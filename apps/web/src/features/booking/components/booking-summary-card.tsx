...
export const BookingSummaryCard = ({
  ...
  const sessionLabel = useMemo(() => {
    return resolveLabel(formValues.sessionType, sessionTypeOptions);
  }, [formValues.sessionType]);

  const durationLabel = useMemo(() => {
    return resolveLabel(formValues.duration, durationOptions);
  }, [formValues.duration]);

  const budgetLabel = useMemo(() => {
    return resolveLabel(formValues.budget, budgetOptions);
  }, [formValues.budget]);
    ...
    <Card>
      <CardContent className="space-y-5">
        ...
        <div className="rounded-2xl border border-border bg-background px-4 py-3">
          ...
          <p className="text-xs text-muted">{photographer.availability}</p>
        </div>
